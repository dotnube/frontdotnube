'use strict';
var bililiteRange; // create one global variable
(function () {
    var datakey = Symbol(); // use as the key to modify elements.
    bililiteRange = function (el) {
        ...
        if (!(el[datakey])) { // we haven't processed this element yet	
            var data_1 = createDataObject(el);
            startupHooks.forEach(function (hook) { return hook(el, ret, data_1); });
        }
        return ret;
    };
    bililiteRange.version = 3.2;
    var startupHooks = new Set();
    bililiteRange.addStartupHook = function (fn) { return startupHooks.add(fn); };
    ...
    function trackSelection(element, range, data) {
        data.selection = [0, 0];
        range.listen('focusout', function (evt) { return data.selection = range._nativeSelection(); });
        range.listen('mousedown', function (evt) { return data.mousetime = evt.timeStamp; });
        range.listen('drop', function (evt) { return data.mousetime = evt.timeStamp; });
        range.listen('focus', function (evt) {
            if ('mousetime' in data && evt.timeStamp - data.mousetime < 100)
                return;
    ...
    function fixInputEvents(element, range, data) {
        ...
        range.listen('input', function (evt) {
            var newText = range.all();
            ...
            data.liveRanges.forEach(function (rng) {
                var start = evt.bililiteRange.start;
                var oldend = start + evt.bililiteRange.oldText.length;
                var newend = start + evt.bililiteRange.newText.length;
                // adjust bounds; this tries to emulate the algorithm that Microsoft Word uses for bookmarks
                var _a = rng.bounds(), b0 = _a[0], b1 = _a[1];
                if (b0 <= start) {
                    // no change
                }
                else if (b0 > oldend) {
    ...
    function diff(oldText, newText) {
        ...
        for (i = 0; i < newlen && i < oldlen; ++i) {
            ...
            if (newpos < start || oldpos < start)
                break;
            if (newText.charAt(newpos) != oldText.charAt(oldpos))
                break;
        }
        var oldend = oldlen - i;
        var newend = newlen - i;
    ...
    function correctNewlines(element, range, data) {
        ...
        range.listen('paste', function (evt) {
    ...
    Range.prototype = {
        ...
        dispatch: function (opts) {
            ...
            ; // ignore read-only errors for properties that were copied in the previous line
        ...
        dontlisten: function (type, func) {
            if (func === void 0) { func = console.log; }
        ...
        scrollIntoView: function (scroller) {
            var _this = this;
            if (scroller === void 0) { scroller = (function (top) { return _this._el.scrollTop = top; }); }
            var top = this.top();
        ...
        sendkeys: function (text) {
            ...
            function simplechar(rng, c) {
                if (/^{[^}]*}$/.test(c))
                    c = c.slice(1, -1); // deal with unknown {key}s
                rng.text(c).bounds('endbounds');
            }
            text.replace(/{[^}]*}|[^{]+|{/g, function (part) { return (bililiteRange.sendkeys[part] || simplechar)(_this, part, simplechar); });
            this.bounds(this.data.sendkeysBounds);
            this.dispatch({ type: 'sendkeys', detail: text });
            return this;
        },
        text: function (text, _a) {
            var _b = (_a === void 0 ? {} : _a).inputType, inputType = _b === void 0 ? 'insertText' : _b;
            if (text !== undefined) {
                var eventparams = [this.text(), text, this[0], inputType];
                this.dispatch(inputEventInit.apply(void 0, ['beforeinput'].concat(eventparams)));
                this._nativeSetText(text, this._nativeRange(this.bounds()));
                this[1] = this[0] + text.length;
                this.dispatch(inputEventInit.apply(void 0, ['input'].concat(eventparams)));
                return this; // allow for chaining
            }
            else {
    ...
    bililiteRange.override = function (name, fn) {
        var oldfn = bililiteRange.prototype[name];
    ...
    bililiteRange.bounds = {
        ...
        union: function (name) {
            ...
            var _a;
            var b = (_a = this.clone()).bounds.apply(_a, rest);
    ...
    bililiteRange.sendkeys = {
        ...
        '{backspace}': function (rng) {
            var b = rng.bounds();
            if (b[0] == b[1])
                rng.bounds([b[0] - 1, b[0]]); // no characters selected; it's just an insertion point. Remove the previous character
            rng.text('', { inputType: 'deleteContentBackward' }); // delete the characters and update the selection
        },
        '{del}': function (rng) {
            var b = rng.bounds();
            if (b[0] == b[1])
                rng.bounds([b[0], b[0] + 1]); // no characters selected; it's just an insertion point. Remove the next character
            rng.text('', { inputType: 'deleteContentForward' }).bounds('endbounds'); // delete the characters and update the selection
        },
        '{rightarrow}': function (rng) {
            var b = rng.bounds();
            if (b[0] == b[1])
                ++b[1]; // no characters selected; it's just an insertion point. Move to the right
            rng.bounds([b[1], b[1]]);
        },
        '{leftarrow}': function (rng) {
            var b = rng.bounds();
            if (b[0] == b[1])
                --b[0]; // no characters selected; it's just an insertion point. Move to the left
            rng.bounds([b[0], b[0]]);
        },
    ...
    InputRange.prototype._nativeTop = function (rng) {
        if (rng[0] == 0)
            return 0; // the range starts at the top
    ...
    function w3cstart(rng, constraint) {
        if (rng.compareBoundaryPoints(START_TO_START, constraint) <= 0)
            return 0; // at or before the beginning
        ...
        return el[datakey] = new Proxy(new Data(el), {
            set: function (obj, prop, value) {
                ...
                if (monitored.has(prop))
                    signalMonitor(prop, value, obj.sourceElement);
    ...
    Object.defineProperty(Data.prototype, 'trigger', {
        value: function () {
            var _this = this;
            monitored.forEach(function (prop) { return signalMonitor(prop, _this[prop], _this.sourceElement); });
        }
    });
